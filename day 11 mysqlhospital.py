...
# =========================
con = __import__('mysql.connector').connector.connect(host="localhost", user="root", password="Venky@15", port=3306)
...
def doctor_login():
    ...
    for d, _, _ in DOCTORS:
        print(f" - {d}")
    name_input = input("Enter your doctor name (exact as above, or Enter to cancel): ").strip()
    if not name_input:
        return
